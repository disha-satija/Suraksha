...
class SafeSpotViewModel extends ChangeNotifier {
  ...
  /// Fetches the 5 nearest safe spots from the SafeSpots_Delhi table.
  /// Falls back to the old OSM/Groq endpoint if the Delhi API returns nothing.
  Future<void> fetchSafeSpots({int radiusKm = 50}) async {
    ...
    try {
      // ── Primary: SafeSpots_Delhi (5 nearest) ──────────────────────────────
      final delhiSpots = await _safeSpotService.findNearestSafeSpots(
        lat: _currentLocation!.latitude,
        lng: _currentLocation!.longitude,
        radiusKm: radiusKm,
      );

      if (delhiSpots.isNotEmpty) {
        _safeSpots = delhiSpots;
        _error = null;
      } else {
        // ── Fallback: OSM / Groq ─────────────────────────────────────────────
  ...
  /// Fetches only the single closest safe spot from SafeSpots_Delhi.
  /// Used by SOS / quick-escape flows.
  Future<SafeSpot?> fetchTopSafeSpot({int radiusKm = 50}) async {
    if (_currentLocation == null) return null;
    try {
      return await _safeSpotService.findTopSafeSpot(
        lat: _currentLocation!.latitude,
        lng: _currentLocation!.longitude,
        radiusKm: radiusKm,
      );
